...
$courseStmt->execute();
$thisCourse = $courseStmt->fetch();
$courseStmt->closeCursor();

$dStmt = $db->prepare('SELECT discussionID FROM usercourselist WHERE courseID=:course_id');
$dStmt->bindValue(':course_id', $user_course_id);
$dStmt->execute();
$thisDiscussion = $dStmt->fetch();
$courseStmt->closeCursor();
...
$courseTutorsStmt->bindValue(':course_id', $user_course_id);
